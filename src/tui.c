#include <ncurses.h>
#include <string.h>
#include <ctype.h>
#include <stdlib.h>
#include <stdio.h>
#include <stdarg.h>
#include "tui.h"
#include "utils.h"
#include "validator.h"
#include "file.h"

/* ===== KONSTANTA LOKAL ===== */
#define MSG_LINE (LINES - 4)    /* Baris untuk menampilkan pesan */

/* ===== FUNGSI HELPER INTERNAL ===== */

/**
 * Menampilkan pesan berwarna di area pesan
 */
static void tampilkan_pesan_berwarna(int color_pair, const char *prefix, const char *fmt, va_list args) {
    char buffer[256], full_msg[300];
    vsnprintf(buffer, sizeof(buffer), fmt, args);
    snprintf(full_msg, sizeof(full_msg), "%s%s", prefix, buffer);

    move(MSG_LINE, 0);
    clrtoeol();

    tui_aktifkan_warna(color_pair);
    tui_cetak_tengah(MSG_LINE, full_msg);
    tui_nonaktifkan_warna(color_pair);

    tui_segarkan();
    tui_ambil_karakter();

    move(MSG_LINE, 0);
    clrtoeol();
}

/* ===== IMPLEMENTASI TAMPILAN & WARNA ===== */

void tui_inisialisasi_warna(void) {
    if (!has_colors()) return;
    init_pair(COLOR_PAIR_CYAN, COLOR_CYAN, COLOR_BLACK);
    init_pair(COLOR_PAIR_RED, COLOR_RED, COLOR_BLACK);
    /* Highlight: Latar Cyan, Teks Hitam */
    init_pair(COLOR_PAIR_HIGHLIGHT, COLOR_BLACK, COLOR_CYAN);
    init_pair(COLOR_PAIR_GREEN, COLOR_GREEN, COLOR_BLACK);
    init_pair(COLOR_PAIR_YELLOW, COLOR_YELLOW, COLOR_BLACK);
}

void tui_aktifkan_warna(int pair_id) { if (has_colors()) attron(COLOR_PAIR(pair_id)); }
void tui_nonaktifkan_warna(int pair_id) { if (has_colors()) attroff(COLOR_PAIR(pair_id)); }
void tui_aktifkan_tebal(void) { attron(A_BOLD); }
void tui_nonaktifkan_tebal(void) { attroff(A_BOLD); }
void tui_aktifkan_terbalik(void) { attron(A_REVERSE); }
void tui_nonaktifkan_terbalik(void) { attroff(A_REVERSE); }
void tui_aktifkan_garis_bawah(void) { attron(A_UNDERLINE); }
void tui_nonaktifkan_garis_bawah(void) { attroff(A_UNDERLINE); }

void tui_cetak_berwarna(int y, int x, int pair_id, const char *text) {
    if (!text) return;
    tui_aktifkan_warna(pair_id);
    mvprintw(y, x, "%s", text);
    tui_nonaktifkan_warna(pair_id);
}

void tui_cetak_tengah_berwarna(int y, int pair_id, const char *text) {
    if (!text) return;
    int len = strlen(text);
    int x = (COLS - len) / 2;
    if (x < 0) x = 0;
    tui_aktifkan_warna(pair_id);
    mvprintw(y, x, "%s", text);
    tui_nonaktifkan_warna(pair_id);
}

void tui_reset_atribut(void) { attrset(A_NORMAL); }

int tui_inisialisasi(void) {
    if (initscr() == NULL) return 0;
    cbreak();
    noecho();
    keypad(stdscr, TRUE);
    curs_set(0);
    if (has_colors()) {
        start_color();
        tui_inisialisasi_warna();
    }
    return 1;
}

void tui_bersihkan(void) { curs_set(1); endwin(); }
void tui_hapus_layar(void) { clear(); }
void tui_segarkan(void) { refresh(); }
int tui_ambil_lebar(void) { return COLS; }
int tui_ambil_tinggi(void) { return LINES; }

/* ===== IMPLEMENTASI PENCETAKAN & GAMBAR ===== */

void tui_cetak(int y, int x, const char *text) {
    if (text) mvprintw(y, x, "%s", text);
}

void tui_cetakf(int y, int x, const char *fmt, ...) {
    if (!fmt) return;
    va_list args;
    va_start(args, fmt);
    move(y, x);
    vw_printw(stdscr, fmt, args);
    va_end(args);
}

void tui_cetak_tengah(int y, const char *text) {
    if (!text) return;
    int x = (COLS - (int)strlen(text)) / 2;
    if (x < 0) x = 0;
    mvprintw(y, x, "%s", text);
}

void tui_gambar_garis_horizontal(int y, int x, int length, char ch) {
    if (ch == 0) ch = '-';
    move(y, x);
    for (int i = 0; i < length; i++) addch(ch);
}

void tui_gambar_kotak(int y, int x, int height, int width) {
    mvaddch(y, x, ACS_ULCORNER);
    for (int i = 1; i < width - 1; i++) addch(ACS_HLINE);
    addch(ACS_URCORNER);
    for (int i = 1; i < height - 1; i++) {
        mvaddch(y + i, x, ACS_VLINE);
        mvaddch(y + i, x + width - 1, ACS_VLINE);
    }
    mvaddch(y + height - 1, x, ACS_LLCORNER);
    for (int i = 1; i < width - 1; i++) addch(ACS_HLINE);
    addch(ACS_LRCORNER);
}

/* ===== IMPLEMENTASI INPUT & KURSOR ===== */

int tui_ambil_karakter(void) { return getch(); }
void tui_atur_kursor(int visible) { curs_set(visible ? 1 : 0); }
void tui_pindah(int y, int x) { move(y, x); }

void tui_jeda(int y) {
    tui_cetak_tengah(y, "Tekan sembarang tombol untuk melanjutkan...");
    tui_segarkan();
    tui_ambil_karakter();
}

void bersihkan_baris_input(int y) {
    move(y, 0);
    clrtoeol();
}

int masukan_string(int y, int x, const char *prompt, char *buffer, int max_len) {
    if (!buffer || max_len <= 0) return 0;
    tui_cetak(y, x, prompt);
    tui_atur_kursor(1);

    int input_x = x + strlen(prompt);
    move(y, input_x);

    char temp[256];
    memset(temp, 0, sizeof(temp));
    int pos = 0, ch;

    while (1) {
        ch = getch();
        if (ch == 27) { tui_atur_kursor(0); buffer[0] = '\0'; return 0; }
        if (ch == '\n' || ch == '\r' || ch == KEY_ENTER) break;
        if (ch == KEY_BACKSPACE || ch == 127 || ch == 8) {
            if (pos > 0) { pos--; temp[pos] = '\0'; mvaddch(y, input_x + pos, ' '); move(y, input_x + pos); }
        } else if (isprint(ch) && pos < max_len - 1 && pos < 254) {
            temp[pos] = ch; mvaddch(y, input_x + pos, ch); pos++; temp[pos] = '\0';
        }
        refresh();
    }
    tui_atur_kursor(0);

    strncpy(buffer, temp, max_len - 1);
    buffer[max_len - 1] = '\0';

    /* Trim whitespace */
    pangkas_string(buffer);

    return 1;
}

int masukan_string_wajib(int y, int x, const char *prompt, char *buffer, int max_len) {
    while (1) {
        if (!masukan_string(y, x, prompt, buffer, max_len)) return 0;
        if (validasi_tidak_kosong(buffer)) return 1;
        tampilkan_error("Input tidak boleh kosong!");
        bersihkan_baris_input(y);
    }
}

int masukan_angka(int y, int x, const char *prompt, unsigned long long *result) {
    char buffer[32];
    while (1) {
        if (!masukan_string(y, x, prompt, buffer, sizeof(buffer))) return 0;
        if (urai_nominal(buffer, result)) return 1;
        tampilkan_error("Input harus berupa angka positif!");
        bersihkan_baris_input(y);
    }
}

int masukan_angka_min(int y, int x, const char *prompt, unsigned long long *result, unsigned long long min) {
    while (1) {
        if (!masukan_angka(y, x, prompt, result)) return 0;
        if (*result >= min) return 1;
        tampilkan_error("Nilai harus minimal %llu!", min);
        bersihkan_baris_input(y);
    }
}

int masukan_tanggal(int y, int x, const char *prompt, char *buffer) {
    char full_prompt[128];
    snprintf(full_prompt, sizeof(full_prompt), "%s (dd-mm-YYYY): ", prompt);
    while (1) {
        if (!masukan_string(y, x, full_prompt, buffer, 11)) return 0;
        if (validasi_tanggal_lengkap(buffer)) return 1;
        tampilkan_error("Format tanggal tidak valid! Gunakan dd-mm-YYYY");
        bersihkan_baris_input(y);
    }
}

int masukan_konfirmasi(int y, int x, const char *prompt) {
    char full_prompt[128];
    snprintf(full_prompt, sizeof(full_prompt), "%s (y/n): ", prompt);
    tui_cetak(y, x, full_prompt);
    tui_segarkan();
    while (1) {
        int ch = tui_ambil_karakter();
        if (ch == 'y' || ch == 'Y') return 1;
        if (ch == 'n' || ch == 'N' || ch == 27) return 0;
    }
}

int masukan_pilihan(int y, int x, const char *prompt, int max) {
    char buffer[16], full_prompt[128];
    snprintf(full_prompt, sizeof(full_prompt), "%s (1-%d): ", prompt, max);
    while (1) {
        if (!masukan_string(y, x, full_prompt, buffer, sizeof(buffer))) return 0;
        int pilihan = atoi(buffer);
        if (validasi_pilihan_menu(pilihan, 1, max)) return pilihan;
        tampilkan_error("Pilihan tidak valid! Masukkan 1-%d", max);
        bersihkan_baris_input(y);
    }
}

int masukan_bulan(int y, int x, const char *prompt, int *result) {
    char buffer[16], full_prompt[128];
    snprintf(full_prompt, sizeof(full_prompt), "%s (1-12): ", prompt);
    while (1) {
        if (!masukan_string(y, x, full_prompt, buffer, sizeof(buffer))) return 0;
        int bulan = atoi(buffer);
        if (bulan >= BULAN_MIN && bulan <= BULAN_MAX) { *result = bulan; return 1; }
        tampilkan_error("Bulan tidak valid! Masukkan 1-12");
        bersihkan_baris_input(y);
    }
}

int masukan_jenis_transaksi(int y, int x, int *result) {
    tui_cetak(y, x, "Jenis: 1=Pengeluaran, 2=Pemasukan: ");
    tui_segarkan();
    while (1) {
        int ch = tui_ambil_karakter();
        if (ch == '1') { *result = JENIS_PENGELUARAN; return 1; }
        if (ch == '2') { *result = JENIS_PEMASUKAN; return 1; }
        if (ch == 27) return 0;
    }
}

/* ===== IMPLEMENTASI SISTEM MENU ===== */

void menu_inisialisasi(Menu *menu, const char *title) {
    if (!menu) return;
    memset(menu, 0, sizeof(Menu));
    if (title) strncpy(menu->title, title, MAX_MENU_TEXT - 1);
    menu->selected = 0;
    menu->start_y = 5;
    menu->start_x = 5;
}

int menu_tambah_item(Menu *menu, const char *text, int value) {
    if (!menu || !text || menu->item_count >= MAX_MENU_ITEMS) return 0;
    strncpy(menu->items[menu->item_count].text, text, MAX_MENU_TEXT - 1);
    menu->items[menu->item_count].value = value;
    menu->item_count++;
    return 1;
}

void menu_atur_posisi(Menu *menu, int y, int x) {
    if (!menu) return;
    menu->start_y = y;
    menu->start_x = x;
}

void menu_tampilkan(Menu *menu) {
    if (!menu) return;
    int y = menu->start_y, x = menu->start_x;

    if (strlen(menu->title) > 0) {
        tui_aktifkan_tebal();
        tui_aktifkan_warna(COLOR_PAIR_CYAN);
        tui_cetak(y, x, menu->title);
        tui_nonaktifkan_warna(COLOR_PAIR_CYAN);
        tui_nonaktifkan_tebal();
        y += 2;
    }

    for (int i = 0; i < menu->item_count; i++) {
        if (i == menu->selected) {
            tui_aktifkan_tebal();
            tui_aktifkan_warna(COLOR_PAIR_HIGHLIGHT);
            tui_cetakf(y + i, x, "> %d. %s  ", i + 1, menu->items[i].text);
            tui_nonaktifkan_warna(COLOR_PAIR_HIGHLIGHT);
            tui_nonaktifkan_tebal();
        } else {
            tui_cetakf(y + i, x, "  %d. %s  ", i + 1, menu->items[i].text);
        }
    }
}

void menu_tampilkan_tengah(Menu *menu) {
    if (!menu) return;
    int max_width = strlen(menu->title);
    for (int i = 0; i < menu->item_count; i++) {
        int len = strlen(menu->items[i].text) + 8;
        if (len > max_width) max_width = len;
    }
    int x = (tui_ambil_lebar() - max_width) / 2;
    int y = (tui_ambil_tinggi() - menu->item_count - 4) / 2;
    if (x < 2) x = 2;
    if (y < 2) y = 2;
    menu_atur_posisi(menu, y, x);
    menu_tampilkan(menu);
}

int menu_navigasi(Menu *menu) {
    if (!menu || menu->item_count == 0) return -1;

    while (1) {
        tui_hapus_layar();
        tampilkan_header("APLIKASI KEUANGAN MAHASISWA");
        menu_tampilkan_tengah(menu);
        tampilkan_footer("ATAS/BAWAH: Navigasi | ENTER: Pilih | ESC: Kembali");
        tui_segarkan();

        int ch = tui_ambil_karakter();
        switch (ch) {
            case KEY_UP: case 'k': case 'K':
                if (menu->selected > 0) menu->selected--;
                break;
            case KEY_DOWN: case 'j': case 'J':
                if (menu->selected < menu->item_count - 1) menu->selected++;
                break;
            case KEY_ENTER: case '\n': case '\r':
                return menu->items[menu->selected].value;
            case 27:
                return CANCEL;
            default:
                if (ch >= '1' && ch <= '9') {
                    int idx = ch - '1';
                    if (idx < menu->item_count) {
                        menu->selected = idx;
                        return menu->items[idx].value;
                    }
                }
                break;
        }
    }
}

int menu_ambil_nilai_terpilih(Menu *menu) {
    if (!menu || menu->item_count == 0) return -1;
    return menu->items[menu->selected].value;
}

/* ===== IMPLEMENTASI UI COMPONENTS ===== */

void tampilkan_header(const char *title) {
    int width = tui_ambil_lebar();
    tui_aktifkan_warna(COLOR_PAIR_CYAN);
    tui_gambar_garis_horizontal(0, 0, width, '=');
    tui_aktifkan_tebal();
    tui_cetak_tengah(1, title);
    tui_nonaktifkan_tebal();
    tui_gambar_garis_horizontal(2, 0, width, '=');
    tui_nonaktifkan_warna(COLOR_PAIR_CYAN);
}

void tampilkan_footer(const char *instructions) {
    int h = tui_ambil_tinggi(), w = tui_ambil_lebar();
    tui_aktifkan_warna(COLOR_PAIR_CYAN);
    tui_gambar_garis_horizontal(h - 2, 0, w, '-');
    if (instructions) tui_cetak_tengah(h - 1, instructions);
    tui_nonaktifkan_warna(COLOR_PAIR_CYAN);
}

void tampilkan_info(const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    tampilkan_pesan_berwarna(COLOR_PAIR_CYAN, "[INFO] ", fmt, args);
    va_end(args);
}

void tampilkan_sukses(const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    tampilkan_pesan_berwarna(COLOR_PAIR_GREEN, "[OK] ", fmt, args);
    va_end(args);
}

void tampilkan_error(const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    tampilkan_pesan_berwarna(COLOR_PAIR_RED, "[ERROR] ", fmt, args);
    va_end(args);
}

void tampilkan_peringatan(const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    tampilkan_pesan_berwarna(COLOR_PAIR_YELLOW, "[PERINGATAN] ", fmt, args);
    va_end(args);
}

int tampilkan_konfirmasi(const char *message) {
    char full_msg[256];
    snprintf(full_msg, sizeof(full_msg), "%s (y/n)", message);
    move(MSG_LINE, 0);
    clrtoeol();
    tui_aktifkan_warna(COLOR_PAIR_YELLOW);
    tui_cetak_tengah(MSG_LINE, full_msg);
    tui_nonaktifkan_warna(COLOR_PAIR_YELLOW);
    tui_segarkan();
    while (1) {
        int ch = tui_ambil_karakter();
        if (ch == 'y' || ch == 'Y') { move(MSG_LINE, 0); clrtoeol(); return 1; }
        if (ch == 'n' || ch == 'N' || ch == 27) { move(MSG_LINE, 0); clrtoeol(); return 0; }
    }
}

void tampilkan_kotak_pesan(const char *title, const char *message, int color_pair) {
    int msg_len = strlen(message);
    int title_len = title ? (int)strlen(title) : 0;
    int width = (msg_len > title_len ? msg_len : title_len) + 6;
    int height = 5;
    int x = (COLS - width) / 2, y = (LINES - height) / 2;
    if (x < 1) x = 1;
    if (y < 1) y = 1;
    tui_aktifkan_warna(color_pair);
    tui_gambar_kotak(y, x, height, width);
    if (title) { tui_aktifkan_tebal(); tui_cetak(y, x + 2, title); tui_nonaktifkan_tebal(); }
    tui_cetak(y + 2, x + 3, message);
    tui_nonaktifkan_warna(color_pair);
    tui_segarkan();
    tui_ambil_karakter();
}

void tampilkan_status(const char *message) {
    if (!message) return;
    move(LINES - 3, 0);
    clrtoeol();
    tui_aktifkan_warna(COLOR_PAIR_CYAN);
    mvprintw(LINES - 3, 2, "%s", message);
    tui_nonaktifkan_warna(COLOR_PAIR_CYAN);
    tui_segarkan();
}

void tampilkan_memuat(const char *message) {
    if (!message) return;
    move(MSG_LINE, 0);
    clrtoeol();
    tui_aktifkan_warna(COLOR_PAIR_YELLOW);
    tui_cetak_tengah(MSG_LINE, message);
    tui_nonaktifkan_warna(COLOR_PAIR_YELLOW);
    tui_segarkan();
}

void bersihkan_area_pesan(void) {
    move(MSG_LINE, 0);
    clrtoeol();
    tui_segarkan();
}

/* ===== IMPLEMENTASI SISTEM TABEL ===== */

void tabel_inisialisasi(Table *table) {
    if (!table) return;
    memset(table, 0, sizeof(Table));
    table->start_y = 4;
    table->start_x = 2;
}

int tabel_tambah_kolom(Table *table, const char *header, int width, int align) {
    if (!table || !header || table->col_count >= MAX_TABLE_COLS) return 0;
    strncpy(table->columns[table->col_count].header, header, MAX_COL_WIDTH - 1);
    table->columns[table->col_count].width = width;
    table->columns[table->col_count].align = align;
    table->col_count++;
    return 1;
}

void tabel_atur_posisi(Table *table, int y, int x) {
    if (!table) return;
    table->start_y = y;
    table->start_x = x;
}

int tabel_gambar_header(Table *table) {
    if (!table) return 0;
    int x = table->start_x;
    int y = table->start_y;

    tui_aktifkan_tebal();
    tui_aktifkan_warna(COLOR_PAIR_CYAN);
    for (int i = 0; i < table->col_count; i++) {
        mvprintw(y, x, "%*s", table->columns[i].width, table->columns[i].header);
        x += table->columns[i].width + 1;
    }
    tui_nonaktifkan_warna(COLOR_PAIR_CYAN);
    tui_nonaktifkan_tebal();

    y++;
    tabel_gambar_pemisah(table, y);

    return y + 1;
}

void tabel_gambar_baris(Table *table, int row, int selected, const char **values) {
    if (!table || !values) return;
    int x = table->start_x;
    int y = table->start_y + 2 + row;

    if (selected) {
        tui_aktifkan_terbalik();
        tui_aktifkan_warna(COLOR_PAIR_HIGHLIGHT);
    }

    for (int i = 0; i < table->col_count; i++) {
        int w = table->columns[i].width;
        const char *val = values[i] ? values[i] : "";

        switch (table->columns[i].align) {
            case ALIGN_RIGHT:
                mvprintw(y, x, "%*s", w, val);
                break;
            case ALIGN_CENTER: {
                int pad = (w - (int)strlen(val)) / 2;
                if (pad < 0) pad = 0;
                mvprintw(y, x + pad, "%*s", w - pad, val);
                break;
            }
            default:
                mvprintw(y, x, "%*s", w, val);
                break;
        }
        x += w + 1;
    }

    if (selected) {
        tui_nonaktifkan_warna(COLOR_PAIR_HIGHLIGHT);
        tui_nonaktifkan_terbalik();
    }
}

void tabel_gambar_pemisah(Table *table, int y) {
    if (!table) return;
    int total_width = 0;
    for (int i = 0; i < table->col_count; i++) {
        total_width += table->columns[i].width + 1;
    }
    tui_gambar_garis_horizontal(y, table->start_x, total_width - 1, '-');
}

int tampilkan_tabel_transaksi(Transaksi *list, int count, int start_y, int selected) {
    Table table;
    tabel_inisialisasi(&table);
    tabel_atur_posisi(&table, start_y, 2);

    tabel_tambah_kolom(&table, "No", 4, ALIGN_RIGHT);
    tabel_tambah_kolom(&table, "ID", 6, ALIGN_LEFT);
    tabel_tambah_kolom(&table, "Tanggal", 12, ALIGN_CENTER);
    tabel_tambah_kolom(&table, "Jenis", 12, ALIGN_LEFT);
    tabel_tambah_kolom(&table, "Pos", 15, ALIGN_LEFT);
    tabel_tambah_kolom(&table, "Nominal", 15, ALIGN_RIGHT);
    tabel_tambah_kolom(&table, "Deskripsi", 20, ALIGN_LEFT);

    int row_y = tabel_gambar_header(&table);

    if (count == 0) {
        tui_cetak(row_y, 4, "(Tidak ada transaksi)");
        return row_y + 2;
    }

    char no_str[16], nominal_str[30];
    const char *values[7];

    for (int i = 0; i < count; i++) {
        snprintf(no_str, sizeof(no_str), "%d", i + 1);
        format_rupiah(list[i].nominal, nominal_str);

        values[0] = no_str;
        values[1] = list[i].id;
        values[2] = list[i].tanggal;
        values[3] = dapatkan_label_jenis(list[i].jenis);
        values[4] = list[i].pos;
        values[5] = nominal_str;
        values[6] = list[i].deskripsi;

        tabel_gambar_baris(&table, i, (i == selected), values);
    }

    return row_y + count + 1;
}

int tampilkan_tabel_pos(PosAnggaran *list, int count, int start_y, int selected) {
    Table table;
    tabel_inisialisasi(&table);
    tabel_atur_posisi(&table, start_y, 2);

    tabel_tambah_kolom(&table, "No", 4, ALIGN_RIGHT);
    tabel_tambah_kolom(&table, "Nama Pos", 20, ALIGN_LEFT);
    tabel_tambah_kolom(&table, "Anggaran", 15, ALIGN_RIGHT);
    tabel_tambah_kolom(&table, "Realisasi", 15, ALIGN_RIGHT);
    tabel_tambah_kolom(&table, "Sisa", 15, ALIGN_RIGHT);
    tabel_tambah_kolom(&table, "Status", 12, ALIGN_CENTER);

    int row_y = tabel_gambar_header(&table);

    if (count == 0) {
        tui_cetak(row_y, 4, "(Tidak ada pos anggaran)");
        return row_y + 2;
    }

    char no_str[16], anggaran_str[30], realisasi_str[30], sisa_str[30];
    const char *values[6];

    for (int i = 0; i < count; i++) {
        snprintf(no_str, sizeof(no_str), "%d", list[i].no);
        format_rupiah(list[i].nominal, anggaran_str);
        format_rupiah(list[i].realisasi, realisasi_str);
        format_saldo(list[i].sisa, sisa_str);

        values[0] = no_str;
        values[1] = list[i].nama;
        values[2] = anggaran_str;
        values[3] = realisasi_str;
        values[4] = sisa_str;
        values[5] = (list[i].status == STATUS_AMAN) ? "Aman" : "Tidak Aman";

        tabel_gambar_baris(&table, i, (i == selected), values);
    }

    return row_y + count + 1;
}
